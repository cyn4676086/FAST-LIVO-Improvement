    ...
    std::list< std::pair<FramePtr, double> >& close_kfs) const
{
    ...
    for (auto& kf : keyframes_)
    {
        ...
        for (size_t cam_id = 0; cam_id < frame->getCameraCount(); ++cam_id)
        {
            // 获取当前关键帧中该相机的所有关键点
            ...
            for (auto& keypoint : kf_keypts)
            {
                ...
                {
                    double dist = (frame->T_f_w_.translation() - kf->T_f_w_.translation()).norm();
...
{
    std::lock_guard<std::mutex> lock(map_mutex_);
    FramePtr furthest_kf = nullptr;
    double maxdist = 0.0;
    for(auto& kf : keyframes_)
    {
        double dist = (kf->pos() - pos).norm();
        if(dist > maxdist) {
            maxdist = dist;
            furthest_kf = kf;
        }
